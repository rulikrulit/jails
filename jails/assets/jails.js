(function() {
  ...
  window.Jails = function(config) {
    ...
    function setUpWsConnection() {
      ...
      JAILS.ws.onerror = onWsError;
      JAILS.ws.onclose = function(e) {

        // TODO: set up retry callbacks
        // var events = Jails.events.onClose = [];
        var timeoutTime = 1000;

        // events.forEach(function(event) {
        //   if (event.method === 'create') {
        //     event.function(e, timeoutTime);
        //   }
        // });

        console.warn('Socket was closed', e);
        setTimeout(function() {
      ...
      JAILS.ws.onmessage = onWsMessage;
    }

    setUpWsConnection();

    JAILS.sendWsMessage = function(request) {

      try {
        var a = JAILS.ws.send(JSON.stringify(request));
        console.log('here', JAILS.ws);
      } catch(e) {
        console.error('Failed to send message. Error handling here');
      }
